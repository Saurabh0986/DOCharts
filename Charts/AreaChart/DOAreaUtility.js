...
const AreaChartUtilities = {
    ...
    leftScaleView: function(dataSource, heightOfScale, widthOfLeftRangePannel){
        var dataRange = dataSource.dataRange;
        var ranges = dataRange.ranges;
        var numberOfRanges = ranges.length;
        var scaleGaps = heightOfScale/numberOfRanges;
        var Arr = ranges.map((a, i) => {
            return <Text style={{height:scaleGaps, textAlignVertical:'bottom', textAlign:'right'}} >{a.label}</Text>
        })

        var adjustHeight = 9;

        return ( <View style={{backgroundColor:'#fff', shadowColor: "#000000", width: widthOfLeftRangePannel,}} >
    ...
    leftScaleShadowView: function(heightOfScale, widthOfLeftRangePannel){
    ...
    backScaleView: function(dataSource, heightOfScale, widthOfScale){
        ...
        var Arr = ranges.map((a, i) => {
            return <View style={{borderBottomColor:"#999", borderBottomWidth:0.2, height:scaleGaps, width: widthOfScale}}/>
        })
